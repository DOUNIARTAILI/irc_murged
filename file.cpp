...
void broadcast(std::vector<Clientx *> &clients, std::string msg)
{
    ...
    while (i < clients.size())
    {
        ...
        if (send(clients[i]->c_fd, msg.c_str(), msg.length(), 0) == -1)
        {
            perror("send");
        }
        i++;
...
void broadcast2(std::list<Clientx> &clients, std::string msg)
{
    ...
    while (i != clients.end())
    {
        if (send(i->c_fd, msg.c_str(), msg.length(), 0) == -1)
        {
            perror("send");
        }
        ++i;
        ...
        while(i < cmd.channel.size())
        {
            ...
            if (it != chan.end())
            {
                ...
                if (it->user_list.size() >= it->max_users && it->mode.find('l') != std::string::npos)
                {
                    ...
                    if (send(client.c_fd, maxusers.c_str(), maxusers.length(), 0) == -1)
                    {
                        perror("send");
                    }
                ...
                if (it->mode.find('i') != std::string::npos && it->is_invite(client.nickname) == false)
                {
                    ...
                    if (send(client.c_fd, inviteonly.c_str(), inviteonly.length(), 0) == -1)
                    {
                        perror("send");
                    }
                ...
                if (it->mode.find('k') != std::string::npos && it->pwd != cmd.channel[i].second)
                {
                    ...
                    if (send(client.c_fd, badpassword.c_str(), badpassword.length(), 0) == -1)
                    {
                        perror("send");
                    }
                ...
                if (!it->is_user(client.nickname))
                {
                    ...
                    if (send(client.c_fd, j_rpl.c_str(), j_rpl.length(), 0) == -1)
                    {
                        perror("send");
                    }
            ...
            else
            {
                if (cmd.channel[i].first[0] != '#')
                {
                    ...
                    if (send(client.c_fd, notonchannel.c_str(), notonchannel.length(), 0) == -1)
                    {
                        perror("send");
                    }
                }                
...
void    broadcast_kick(std::vector<Channel>::iterator it, Clientx &user, std::string &cli, std::string &reason)
{
    for (size_t i = 0; i < it->user_list.size(); ++i)
    {
        ...
        if (send(it->user_list[i]->c_fd, syn.c_str(), syn.length(), 0) == -1)
        {
            perror("send");
        }
    }
    ...
    if (cmd.command_arg.size() > 0)
    {
            ...
            if (it != chan.end())
            {
                ...
                else
                {
                    ...
                    if (send(client.c_fd, chanoprivsneeded.c_str(), chanoprivsneeded.length(), 0) == -1)
                    {
                        perror("send");
                    }
                }
            ...
            else
            {
                ...
                if (send(client.c_fd, nosuchchannel.c_str(), nosuchchannel.length(), 0) == -1)
                {
                    perror("send");
                }
            }
        ...
        if (it != clients.end())
        {
            ...
            if (it2 != chan.end())
            {
                if (it2->is_operator(client.nickname))
                {
                    ...
                    if (send(it->c_fd, sendinv.c_str(), sendinv.length(), 0) == -1)
                    {
                        perror("send");
                    }
...
void nick(std::vector<Channel>&chan, Command &cmd, Clientx &client, std::list<Clientx> &clients)
{
    ...
    if (cmd.command_arg.size() > 0)
    {
        ...
        else
        {
            ...
            if (send(client.c_fd, nickinuse.c_str(), nickinuse.length(), 0) == -1)
            {
                perror("send");
            }
        ...
        while(i < cmd.privmsg_list.size())
        {
                ...
                if (it != chan.end())
                {
                    ...
                    while (j < it->user_list.size())
                    {
                        if (it->user_list[j]->nickname != client.nickname)
                        {
                            ...
                            if (send(it->user_list[j]->c_fd, prvmsg.c_str(), prvmsg.length(), 0) == -1)
                            {
                                perror("send");
                            }
                        }
            ...
            {   
                ...
                if (it != clients.end())
                {
                    ...
                    if (send(it->c_fd, privmsg.c_str(), privmsg.length(), 0) == -1)
                    {
                        perror("send");
                    }
                }
        ...
        while(i < cmd.channel.size())
        {
            ...
            if (it != chan.end())
            {
                ...
                if (cmd.command_arg.size() == 1 && !chan[i].topic.empty())
                {
                    ...
                    if (send(client.c_fd, topicmsg.c_str(), topicmsg.length(), 0) == -1)
                    {
                        perror("send");
                    }
                    ...
                    if (send(client.c_fd, topictime.c_str(), topictime.length(), 0) == -1)
                    {
                        perror("send");
                    }
                    return ;
    ...
    if (cmd.command_arg.size()  == 1)
    {
        ...
        if (it != chan.end())
        {
            ...
            if (it->is_user(client.nickname))
            {
                ...
                if (send(client.c_fd, mode.c_str(), mode.length(), 0) == -1)
                {
                    perror("send");
                }
                return ;
            ...
            if (it->is_operator(client.nickname))
            {
                while (i < cmd.mode.size())
                {
                            ...
                            if (cmd.mode[i].first == '+')
                            {
                                if (cmd.mode_args.size() > x)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (send(client.c_fd, user_not_found.c_str(), user_not_found.length(), 0) == -1)
                                        {
                                            perror("send");
                                        }
                                    }
                        ...
                        if (cmd.mode[i].second == 'k')
                        {
                            if (cmd.mode[i].first == '+') 
                            {
                                if (cmd.mode_args.size() > x)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (send(client.c_fd, alreadyset.c_str(), alreadyset.length(), 0) == -1)
                                        {
                                            perror("send");
                                        }
                                    }
                            ...
                            else if (cmd.mode[i].first == '-')
                            {
                                if (!it->k_flag)
                                {
                                    ...
                                    if (send(client.c_fd, keynotset.c_str(), keynotset.length(), 0) == -1)
                                    {
                                        perror("send");
                                    }
                                }
                    ...
                    if (cmd.mode[i].second != 'i' && cmd.mode[i].second != 't' && cmd.mode[i].second != 'l' && cmd.mode[i].second != 'o' && cmd.mode[i].second != 'k')
                    {
                        ...
                        if (send(client.c_fd, unknownmode.c_str(), unknownmode.length(), 0) == -1)
                        {
                            perror("send");
                        }
                    }
